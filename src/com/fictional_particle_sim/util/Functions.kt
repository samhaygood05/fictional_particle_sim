...
object Functions {
    ...
    fun random(): Particle {
        return Particle(pos = Point(Math.random() * Constants.SCALE_WIDTH, Math.random() * Constants.SCALE_HEIGHT), charge = Math.random() * 2 - 1)
    }
    fun random(mass: Double): Particle {
        return Particle(pos = Point(Math.random() * Constants.SCALE_WIDTH, Math.random() * Constants.SCALE_HEIGHT), mass = mass, charge = Math.random() * 2 - 1)
    }
    fun random(vel: Vector, mass: Double): Particle {
        return Particle(pos = Point(Math.random() * Constants.SCALE_WIDTH, Math.random() * Constants.SCALE_HEIGHT), vel = vel, mass = mass, charge = Math.random() * 2 - 1)
    }
    fun random(mass: Double, charge: Double): Particle {
